...
## 触发条件
当检测到用户提交的代码、配置文件或文档（如 README.md、settings.json、.env、config 文件、脚本等）中包含 Windows 或 Unix 风格的绝对路径（如 C:/Users/...、C:\Users\...、/home/...、/Users/...、/root/...）且路径中包含用户名、家目录或本地文件结构信息时触发。特别关注路径中是否包含真实用户名（如 john、admin）或占位符（如 <你的用户名>、<username>、<user>、YourName）。若多次提交反复出现该模式（如经验 213、143、138、92、80、72），视为高频风险，应自动触发 Skill。同时，检查是否有 .gitignore 或 .env 文件缺失，导致敏感文件可能被提交。
...
- **上下文分析**：检查路径中是否包含疑似用户名、用户目录名或占位符（如 `<你的用户名>`、`<username>`、`<user>`、`YourName`），这些是硬编码路径的典型特征。
...
## 元数据
- 版本: 10
- 创建时间: 2026-05-26T20:53:30.083462
- 来源: 自动合并
