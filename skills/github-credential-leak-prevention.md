...
## 触发条件
在分析 GitHub 提交历史、当前仓库文件或用户代码/配置文件/文档时，检测到包含硬编码的敏感信息（如 Windows 绝对路径、用户名、密码、API 密钥、令牌、本地路径、配置文件占位符等）被提交到版本控制中，或存在于文本文件中。

## 行为规则
### 1. 检测方法
- 扫描所有文本文件（.md, .json, .yml, .py, .sh, .env 等）以及提交历史和仓库内容，使用正则表达式匹配常见敏感信息模式，包括但不限于：
  - **Windows 绝对路径**：`[A-Za-z]:/Users/`、`C:/Users/`、`C:\Users\...`、`C:\Users\w+`。
  - **用户名和占位符**：`<你的用户名>`、`<your-username>`、`<username>` 等未替换的占位符。
...
- **替换为环境变量或相对路径**：
  - 将硬编码的绝对路径替换为相对路径或环境变量引用（如 `%USERPROFILE%`、`$HOME`、`%USERNAME%`）。
  - 将硬编码的敏感信息替换为环境变量或配置文件模板。
- **配置文件模板化**：创建 `.env.example` 或 `config.template.json`，将真实值替换为占位符，并确保凭据已修改或撤销。
...
- 用户项目 'evomentor-agent' 的 README.md 中出现 `C:/User/...` 路径，暴露了开发者用户名。
...
## 元数据
- 版本: 4
- 创建时间: 2026-05-22T14:50:08.258052
- 来源: 自动合并
