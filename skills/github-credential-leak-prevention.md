...
## 触发条件
在分析 GitHub 提交历史、当前仓库文件、代码审查或配置文件扫描中，检测到包含硬编码的敏感信息（包括但不限于 Windows 或 Unix 绝对路径、用户名、密码、API 密钥、令牌、本地路径、配置文件占位符等）被提交到版本控制中，或存在于文本文件中，且可能暴露个人用户名、目录结构或凭证信息。
...
- 扫描所有文本文件（.md, .json, .yml, .py, .sh, .env 等）以及提交历史和仓库内容，使用正则表达式匹配常见敏感信息模式，包括但不限于：
  - **Windows 绝对路径**：`[A-Za-z]:\Users\`、`[A-Za-z]:/Users/`、`C:/Users/`、`C:\Users\...`、`C:\Users\w+`。
  - **Unix 绝对路径**：`/home/`、`/Users/` 后跟具体用户名，以及其他常见绝对路径如 `/var/`、`/etc/` 等。
...
- **替换为环境变量或相对路径**：
  - 将硬编码的绝对路径替换为相对路径（如 `./data/`、`./assets/`）或使用环境变量（如 `%USERPROFILE%`、`$HOME`、`%USERNAME%`、`$PROJECT_ROOT`）。
  - 将硬编码的敏感信息替换为环境变量或配置文件模板。
- **配置文件模板化**：创建 `.env.example`、`config.template.json` 或 `settings.template.json`，将真实值替换为通用占位符（如 `<YOUR_USERNAME>`、`<PROJECT_ROOT>`、`<your-home-dir>`），并确保凭据已修改或撤销。
- **更新 `.gitignore` 或 `.dockerignore`**：添加配置文件路径（如 `settings.json`、`.env`、`*.key`），排除包含敏感路径的本地配置文件，防止未来提交。
...
- 用户 'claude-pause-chime' 项目修复了文档中的个人绝对路径，替换为通用占位符。
- 常见错误：在 CI/CD 配置中直接写入本地路径，导致跨平台构建失败。
- 经验 ID 92, 72, 143, 138, 80 均涉及敏感信息提交到版本控制，需确保规则覆盖更全面的敏感信息类型（如密钥、令牌、本地路径、占位符等）。

## 元数据
- 版本: 7
- 创建时间: 2026-05-26T21:55:00.375026
- 来源: 自动合并
