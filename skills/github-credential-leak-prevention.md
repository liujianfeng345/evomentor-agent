...
- **替换为环境变量或相对路径**：
  - 将硬编码的绝对路径替换为相对路径（如 `./data/`、`./config`）或使用环境变量（如 `%USERPROFILE%`、`$HOME`、`%USERNAME%`）。
  - 将硬编码的敏感信息替换为环境变量或配置文件模板。
- **配置文件模板化**：创建 `.env.example`、`config.template.json` 或 `settings.template.json`，将真实值替换为占位符（如 `<your-home-dir>`），并确保凭据已修改或撤销。
...
## 元数据
- 版本: 6
- 创建时间: 2026-05-26T17:53:22.600477
- 来源: 自动合并
